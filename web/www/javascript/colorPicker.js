function ColorPicker(elemPicker, elemGradient, elemColor, input) {
  ...
  self.__init = function(elemPicker, elemGradient, elemColor, input) {
  ...
  self.colorInRgb = function(hexa) {
    if (hexa.length == 7 && hexa[0] == '#') {
      var newColor = [self.convertToDec(hexa[2] + hexa[1]),
                      self.convertToDec(hexa[4] + hexa[3]),
                      self.convertToDec(hexa[6] + hexa[5])];
    } else if (hexa.length == 6) {
      var newColor = [self.convertToDec(hexa[1] + hexa[0]),
  ...
  self.__init(elemPicker, elemGradient, elemColor, input);
};
